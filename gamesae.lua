...
--// + CLIENT RENDER / EGG RENDER REMOVER
--//============================================================//
...
--//============================================================//
--// RESPONSIVE PHONE / PC UI SCALE
--// Keeps the panel fitted on small phone screens.
--//============================================================//
...
local function UpdateMainScale()
    ...
    -- Keep the original size on PC/tablet, but shrink on phones.
    local Scale = 1

    if Width <= 500 then
        Scale = math.min((Width - 24) / 350, (Height - 24) / 435)
        Scale = math.clamp(Scale, 0.62, 0.95)
    elseif Width <= 700 then
        Scale = math.min((Width - 40) / 350, (Height - 40) / 435)
        Scale = math.clamp(Scale, 0.78, 1)
    end

    MainScale.Scale = Scale

    -- Center the scaled panel.
    Main.Position = UDim2.new(0.5, -175 * Scale, 0.5, -217.5 * Scale)
end

UpdateMainScale()

local CameraChangedConnection
local function ConnectResponsiveUI()
    ...
    local Camera = workspace.CurrentCamera
    if Camera then
        CameraChangedConnection = Camera:GetPropertyChangedSignal("ViewportSize"):Connect(UpdateMainScale)
    end
end
...
local function ProtectHumanoid(Humanoid)
	...
	GodHealthConnection = Humanoid.HealthChanged:Connect(function()
...
local function EnableGodMode()
	...
	GodModeButton.TextColor3 = Color3.fromRGB(100,255,130)
	GodModeButton.BackgroundColor3 = Color3.fromRGB(35,75,48)
...
local function DisableGodMode()
	...
	if CurrentGodHumanoid and CurrentGodHumanoid.Parent then
		pcall(function()
...
local function StartFloat()
	...
	FloatConnection = RunService.Heartbeat:Connect(function()
		...
		local Ground = FindGround(Character,Root)
...
FloatButton.MouseButton1Click:Connect(function()
	...
	if FloatEnabled then
		FloatButton.Text = "FLOAT 0.5: ON"
		FloatButton.TextColor3 = Color3.fromRGB(100,255,130)
		FloatButton.BackgroundColor3 = Color3.fromRGB(35,75,48)

		StartFloat()

		StatusLabel.Text = "● Float 0.5 enabled"
		StatusLabel.TextColor3 = Color3.fromRGB(100,255,130)
	else
		StopFloat()
...
--//============================================================//
--// REMOVE CLIENT PET / EGG RENDER FOLDERS
--//============================================================//
...
local function RestoreRenderFolders()
	for Object,Data in pairs(SavedRenderFolders) do
		if Object and Data and Data.Parent then
			pcall(function()
...
local function HidePart(Object)
	...
	if not SavedParts[Object] then
		SavedParts[Object] = {
			Transparency = Object.LocalTransparencyModifier,
...
local function ApplyBoost(Object)
	...
	-- Render folders are handled separately.
	if IsRenderFolder(Object) then
		RemoveRenderFolder(Object)
		return
	end
...
local function EnableFPSBoost()
	...
	FPSBoostButton.TextColor3 = Color3.fromRGB(100,255,130)
	FPSBoostButton.BackgroundColor3 = Color3.fromRGB(35,75,48)

	StatusLabel.Text = "● FPS Boost enabled"
	StatusLabel.TextColor3 = Color3.fromRGB(100,255,130)

	ApplyLightingBoost()

	--// REMOVE PET + EGG RENDER FOLDERS
	RemoveRenderFolders()

	--// APPLY NORMAL VISUAL BOOST
	for _,Object in ipairs(workspace:GetDescendants()) do
		ApplyBoost(Object)
	end
end
...
local function DisableFPSBoost()
	...
	for Object,Data in pairs(SavedParts) do
		if Object and Object.Parent then
			pcall(function()
				Object.LocalTransparencyModifier = Data.Transparency
	...
	--// RESTORE PET + EGG RENDER FOLDERS
	RestoreRenderFolders()

	StatusLabel.Text = "● FPS Boost disabled"
	StatusLabel.TextColor3 = Color3.fromRGB(170,170,180)
end
...
local function FreezeGuard(Model)
	...
	local Data = {
		...
		JumpHeight = Humanoid and Humanoid.JumpHeight,
	...
	for _,Object in ipairs(Model:GetDescendants()) do
		if Object:IsA("BasePart") then
			Data.Parts[Object] = Object.Anchored
...
local function UnfreezeGuard(Model,Data)
	...
	if Data.Humanoid and Data.Humanoid.Parent then
		pcall(function()
			Data.Humanoid.WalkSpeed = Data.WalkSpeed
			Data.Humanoid.JumpPower = Data.JumpPower
			Data.Humanoid.JumpHeight = Data.JumpHeight
...
local function FreezeAllGuards()
	...
	if not GuardFolder then
		StatusLabel.Text = "● GuardAreas not found"
	...
	StatusLabel.Text = "● Frozen Guards: " .. Count
	StatusLabel.TextColor3 = Color3.fromRGB(100,255,130)
end
...
local function UnfreezeAllGuards()
	...
	StatusLabel.Text = "● Guards unfrozen"
	StatusLabel.TextColor3 = Color3.fromRGB(170,170,180)
end
...
GuardFreezeButton.MouseButton1Click:Connect(function()
	if GuardFreezeEnabled then
		...
		GuardFreezeButton.Text = "FREEZE GUARDS: ON"
		GuardFreezeButton.TextColor3 = Color3.fromRGB(100,255,130)
		GuardFreezeButton.BackgroundColor3 = Color3.fromRGB(35,75,48)
	end
end)
...
local function IsPlayerModel(Model)
	...
	for _,OtherPlayer in ipairs(Players:GetPlayers()) do
		if OtherPlayer.Character
			and Model:IsDescendantOf(OtherPlayer.Character)
		then
			return true
		end
	end
...
local function FreezeAllAI()
	...
	StatusLabel.Text = "● AI Speed 0: " .. Count
	StatusLabel.TextColor3 = Color3.fromRGB(100,255,130)
end
...
local function UnfreezeAllAI()
	...
	StatusLabel.Text = "● AI speed restored"
	StatusLabel.TextColor3 = Color3.fromRGB(170,170,180)
end
...
AIFreezeButton.MouseButton1Click:Connect(function()
	if AIFreezeEnabled then
		...
		AIFreezeButton.Text = "AI FREEZE: ON"
		AIFreezeButton.TextColor3 = Color3.fromRGB(100,255,130)
		AIFreezeButton.BackgroundColor3 = Color3.fromRGB(35,75,48)
	end
end)
...
workspace.DescendantAdded:Connect(function(Object)
	...
	task.defer(function()
		...
		if Model and IsAIModel(Model) then
			FreezeAI(Model)
		end
...
workspace.DescendantAdded:Connect(function(Object)
	...
	task.defer(function()
		...
		--// If the game creates ClientRenderedAssets
		--// or PlacedEggRenders again, remove them locally.
		if IsRenderFolder(Object) then
			RemoveRenderFolder(Object)
			return
		end

		--// Also check the ancestor in case a child
		--// was created before the folder handler runs.
		local Ancestor = Object:FindFirstAncestor(function(Item)
			return IsRenderFolder(Item)
		end)

		if Ancestor then
			RemoveRenderFolder(Ancestor)
			return
		end
			...
			if Folder then
				...
				for Model,Data in pairs(FrozenGuards) do
					...
					then
						...
						for Part,OldAnchored in pairs(Data.Parts) do
...
--//============================================================//
--// 🏏 BAT AUTO DETECTION
--// Automatically activates the Bat when a target is within 20 studs
--//============================================================//

local BatAutoEnabled = false
local BatDetectionDistance = 100
local BatCooldown = 0.5
...
local function GetBat()
    ...
    for _, Object in ipairs(Character:GetChildren()) do
        if Object:IsA("Tool") then
            if string.lower(Object.Name):find("bat") then
                return Object
            end
        end
    end
...
BatAutoButton.MouseButton1Click:Connect(function()
    ...
    if BatAutoEnabled then
        ...
        BatAutoButton.Text = "BAT AUTO: ON"
        ...
        StatusLabel.Text = "● Bat Auto enabled"
        StatusLabel.TextColor3 =
            Color3.fromRGB(100,255,130)

    else

        BatAutoButton.Text = "BAT AUTO: OFF"
        BatAutoButton.TextColor3 =
        ...
        StatusLabel.Text = "● Bat Auto disabled"
        StatusLabel.TextColor3 =
...
--//============================================================//*
--// 📷 FAR CAMERA
--// Forces the camera to a specific third-person distance
--//============================================================//*

local FarCameraEnabled = false

--// Set the distance you want here.
--// 10 = close
--// 25 = medium
--// 50 = far
--// 100 = very far
...
local function StartFarCamera()
    ...
    -- Save the player's original camera settings
    SavedMinZoomDistance = Player.CameraMinZoomDistance
    SavedMaxZoomDistance = Player.CameraMaxZoomDistance

    -- Force the camera to the selected distance
    Player.CameraMinZoomDistance = FarCameraDistance
    Player.CameraMaxZoomDistance = FarCameraDistance

    -- Make sure the camera is in third person
    Player.CameraMode = Enum.CameraMode.Classic

    FarCameraConnection = RunService.RenderStepped:Connect(function()
        ...
        -- Keep the camera settings locked
        Player.CameraMode = Enum.CameraMode.Classic
        Player.CameraMinZoomDistance = FarCameraDistance
        Player.CameraMaxZoomDistance = FarCameraDistance
...
FarCameraButton.MouseButton1Click:Connect(function()
    ...
    FarCameraEnabled = not FarCameraEnabled

    if FarCameraEnabled then

        FarCameraButton.Text = "FAR CAMERA: ON"
        ...
        StatusLabel.Text =
            "● Far Camera: " .. FarCameraDistance .. " studs"
        ...
        FarCameraButton.Text = "FAR CAMERA: OFF"
        ...
        StatusLabel.Text = "● Far Camera disabled"
...
ScreenGui.Destroying:Connect(function()
	...
	--// EXTRA SAFETY:
	--// restore detached pet/egg folders if any remain.
	RestoreRenderFolders()
...
local function SetAutoOnState(Button, Text)
    if Button then
        Button.Text = Text
        Button.TextColor3 = Color3.fromRGB(100,255,130)
        Button.BackgroundColor3 = Color3.fromRGB(35,75,48)
    end
end

task.spawn(function()
    -- Give the character and game folders a moment to load.
    task.wait(1)
    ...
    if not FloatEnabled then
        FloatEnabled = true
        SetAutoOnState(FloatButton, "FLOAT 0.5: ON")
        StartFloat()
    end
    ...
    if not GuardFreezeEnabled then
        GuardFreezeEnabled = true
        FreezeAllGuards()
        SetAutoOnState(GuardFreezeButton, "FREEZE GUARDS: ON")
    end
    ...
    if not AIFreezeEnabled then
        AIFreezeEnabled = true
        FreezeAllAI()
        SetAutoOnState(AIFreezeButton, "AI FREEZE: ON")
    end

    -- BAT AUTO
    if not BatAutoEnabled then
        BatAutoEnabled = true
        SetAutoOnState(BatAutoButton, "BAT AUTO: ON")
    end

    -- FAR CAMERA
    if not FarCameraEnabled then
        FarCameraEnabled = true
        SetAutoOnState(FarCameraButton, "FAR CAMERA: ON")
        StartFarCamera()
    end

    -- Open FPS page so the player immediately sees FPS, ping and ON states.
    HomePage.Visible = false
    FPSPage.Visible = true
    ...
    StatusLabel.Text = "● ALL FEATURES ACTIVE"
    StatusLabel.TextColor3 = Color3.fromRGB(100,255,130)

    print("🥚 EGG FPS MONITOR — ALL FEATURES AUTO ON")
end)
